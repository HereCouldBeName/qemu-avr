...
struct avr_opcodes_s
{
  ...
  const char *opcode;
  const int insn_size;		/* In words.  */
  const int isa;
  const unsigned int bin_opcode;
};
  ...
  *sym = 0;
  ...
  switch (constraint)
	...
	*buf++ = '+';
    ...
    case 'n':
      sprintf (buf, "??");
      fprintf (stderr, "Internal disassembler error");
      ok = 0;
      break;
    ...
    default:
      sprintf (buf, "??");
      fprintf (stderr, "unknown constraint `%c'", constraint);
      ok = 0;
    }
...
print_insn_avr (bfd_vma addr, disassemble_info *info)
{
  ...
  fprintf_function prin = info->fprintf_func;
  ...
  if (!initialized)
    {
	 ...
	 is no symbol table available.  */
      ...
      avr_bin_masks = malloc (nopcodes * sizeof (unsigned int));
